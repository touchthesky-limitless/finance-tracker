			...
			<div className="max-w-5xl mx-auto space-y-10">
				...
				<header className="pt-8 pb-4">
					...
					<p className="text-lg md:text-xl text-gray-500 dark:text-gray-400 max-w-2xl leading-relaxed">
						Your auto insurance hits tomorrow, but you still have a comfortable{" "}
						<strong className="text-gray-900 dark:text-white font-bold">
							$425.50
						</strong>{" "}
						safe to spend before Friday&apos;s paycheck.
					</p>
				</header>

				{/* FIX: Shifted the grid breakpoint from lg (1024px) to xl (1280px) for better iPad viewing */}
					...
					<div className="xl:col-span-7 space-y-8">
						...
						<div className="relative overflow-hidden bg-white dark:bg-[#121212] rounded-4xl p-8 md:p-10 shadow-sm border border-gray-100 dark:border-gray-800/60">
							...
							<h2 className="text-6xl md:text-7xl font-black text-gray-900 dark:text-white tracking-tighter mb-6">
								$425
								<span className="text-3xl md:text-4xl text-gray-400 dark:text-gray-600">
									.50
								</span>
							</h2>
							...
							<div className="space-y-2">
								...
								<div className="h-3 w-full bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
									{/* Time indicator (gray) */}
									<div
										className="h-full bg-gray-300 dark:bg-gray-600 rounded-full absolute"
										style={{ width: "80%" }}
									></div>
					...
					<div className="xl:col-span-5 space-y-6">
						...
						<div className="bg-white dark:bg-[#121212] rounded-4xl p-6 border border-gray-100 dark:border-gray-800/60 shadow-sm">
							<h3 className="text-lg font-bold text-gray-900 dark:text-white mb-6 px-2 flex items-center justify-between">
								Action Items
								<span className="bg-red-100 text-red-600 dark:bg-red-500/10 dark:text-red-400 text-xs px-2 py-1 rounded-full">
									2 Pending
								</span>
							</h3>
							...
							<div className="space-y-3">
								...
								<div className="group flex items-start gap-4 p-4 rounded-2xl hover:bg-orange-50 dark:hover:bg-orange-500/5 border border-transparent hover:border-orange-100 dark:hover:border-orange-500/20 transition-colors cursor-pointer">
									...
									<div className="flex-1">
										<h4 className="text-sm font-bold text-gray-900 dark:text-white mb-1">
											Coffee budget exceeded
										</h4>
										<p className="text-xs text-gray-500 leading-relaxed">
											You hit your $50 limit for the month. Would you like to
											pull $15 from &quot;Dining Out&quot; to cover the overage?
										</p>
										<div className="flex gap-2 mt-3">
											<button className="text-xs font-bold px-3 py-1.5 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-lg">
												Rebalance
											</button>
											<button className="text-xs font-bold px-3 py-1.5 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg">
												Ignore
											</button>
										</div>
									</div>
								...
								<div className="group flex items-start gap-4 p-4 rounded-2xl hover:bg-gray-50 dark:hover:bg-gray-800/50 border border-transparent hover:border-gray-200 dark:hover:border-gray-700 transition-colors cursor-pointer">
									...
									<div className="flex-1">
										...
										<p className="text-xs text-gray-500 leading-relaxed">
											We categorized your recent purchases. Take a quick look to
...
function RunwayDay({ day, date, state, amount }: RunwayDayProps) {
	...
	let bgStyle =
		"bg-white dark:bg-[#121212] border-gray-100 dark:border-gray-800";
	let textStyle = "text-gray-900 dark:text-white";
	let indicator = null;

	if (state === "bill") {
		indicator = (
			<div className="w-2 h-2 rounded-full bg-red-500 mb-2 mt-auto"></div>
		);
		bgStyle =
			"bg-white dark:bg-[#121212] border-red-100 dark:border-red-900/30";
	} else if (state === "income") {
		indicator = (
			<div className="w-2 h-2 rounded-full bg-green-500 mb-2 mt-auto"></div>
		);
		bgStyle =
		...
		indicator = (
			<div className="w-1.5 h-1.5 rounded-full bg-gray-200 dark:bg-gray-800 mb-2 mt-auto"></div>
		);
	}
	...
	return (
		<div
			className={`flex flex-col items-center shrink-0 w-20 h-28 rounded-2xl border ${bgStyle} p-3 transition-transform hover:-translate-y-1 cursor-pointer`}
		>
			<span
				className={`text-[10px] font-bold uppercase tracking-wider opacity-70 ${textStyle}`}
			>
				{day}
			</span>
			...
			{amount && (
				<span
					className={`text-[9px] font-bold truncate w-full text-center ${state === "bill" ? "text-red-500" : "text-green-600 dark:text-green-400"}`}
				>
					{amount}
				</span>
			)}
