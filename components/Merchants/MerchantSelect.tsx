/**
 * Searchable dropdown for selecting a merchant.
 * Supports fuzzy search, creation, keyboard navigation, and floating dropdown.
 */
"use client";
...
import { MerchantOptionContent } from "./MerchantOptionContent";
...
export function MerchantSelect({
	...
	const visibleMerchants = useMemo(() => {
		...
		const matches = normalizedQuery
			? options.filter((merchant) =>
					normalizeMerchantName(merchant.name).includes(normalizedQuery),
				)
			: options;
	...
	const exactMerchant = useMemo(() => {
		...
		return (
			options.find(
				(merchant) => normalizeMerchantName(merchant.name) === normalizedQuery,
			) ?? null
		);
	}, [options, query]);
	...
	const setReferenceElement = useCallback(
		(element: HTMLInputElement | null) => refs.setReference(element),
		[refs],
	);
	const setFloatingElement = useCallback(
		(element: HTMLDivElement | null) => refs.setFloating(element),
		[refs],
	);
	...
	const selectMerchant = (merchant: MerchantListItem) => {
		onChange({ id: merchant.id, name: merchant.name });
		setDraftQuery(null);
		setActiveIndex(0);
		setCreateError(null);
		setIsOpen(false);
	...
	const createMerchant = async () => {
		...
		if (!cleanName || !canCreate || isCreating) return;
			...
			{isOpen && (
				<FloatingPortal>
					<div
						...
						className="z-[10000] overflow-y-auto rounded-xl border border-gray-200 bg-white p-1.5 shadow-[0_18px_50px_rgba(0,0,0,0.22)] outline-none dark:border-white/10 dark:bg-[#202020]"
					>
						{visibleMerchants.map((merchant, index) => {
							...
							return (
								<button
									...
									className={`
                    flex w-full items-center rounded-lg px-3 py-2 text-left outline-none transition-colors
                    ${
											active
                  ...
                  `}
						...
						{canCreate && (
							<button
								...
								aria-selected={false}
								disabled={isCreating}
								onMouseEnter={() => setActiveIndex(visibleMerchants.length)}
								onMouseDown={(event) => event.preventDefault()}
								onClick={() => void createMerchant()}
								className={`
                  mt-1 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-semibold
                  text-[#FF5A35] outline-none transition-colors hover:bg-[#FF5A35]/8
                  disabled:cursor-not-allowed disabled:opacity-50
                  ${activeIndex === visibleMerchants.length ? "bg-[#FF5A35]/8" : ""}
                `}
							>
								<span className="grid size-8 shrink-0 place-items-center rounded-lg bg-[#FF5A35]/10">
									{isCreating ? (
						...
						{createError && (
							<p className="mt-1 border-t border-gray-200 px-3 py-2 text-sm text-red-500 dark:border-white/10">
								{createError}
							</p>
						)}
					</div>
