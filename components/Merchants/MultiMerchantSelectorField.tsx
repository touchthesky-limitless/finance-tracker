/**
 * A multi-select dropdown for merchants, using Radix Popover.
 * Allows selecting multiple merchants, with search and "clear all".
 */
"use client";
...
export function MultiMerchantSelectorField({
	...
	const validMerchants = useMemo(() => {
		...
		for (const merchant of merchants) {
			...
			if (!current || merchant.transactionCount > current.transactionCount) {
				byId.set(id, { ...merchant, id, name });
			}
		}
	...
	const merchantById = useMemo(() => {
		return new Map(validMerchants.map((merchant) => [merchant.id, merchant]));
	}, [validMerchants]);

	const selectedMerchants = useMemo(() => {
		return values
			.map((id) => merchantById.get(id))
			.filter((merchant): merchant is MerchantListItem => Boolean(merchant));
	}, [merchantById, values]);

	const visibleMerchants = useMemo(() => {
		const normalizedQuery = normalize(query);
		if (!normalizedQuery) return validMerchants;
		return validMerchants.filter((merchant) =>
			normalize(merchant.name).includes(normalizedQuery),
		);
	}, [query, validMerchants]);

	const toggleMerchant = (merchantId: string): void => {
		onChange(
			values.includes(merchantId)
				? values.filter((id) => id !== merchantId)
				: [...values, merchantId],
		);
	};
				...
				>
					...
					<div className="max-h-96 overflow-y-auto p-3">
						...
						>
							All merchants
							{values.length === 0 && (
								<Check size={19} strokeWidth={2.5} className="text-[#FF5A35]" />
							)}
						</button>
						...
						{validMerchants.length > 0 && visibleMerchants.length === 0 && (
