...
import { useGoalAllocations } from "@/hooks/goals/useGoalAllocations";
import { useGoalAccountLinks } from "@/hooks/goals/useGoalAccountLinks";
...
import { addMonths, buildContributionData } from "@/lib/goals/utils";
import { MetricCard } from "@/components/Goals/SharedComponents";
...
export function SavingsDetailView() {
	...
	} = useGoalAllocations(goalId);
	const {
		links: accountLinks,
		loading: linksLoading,
		error: linksError,
		reload: reloadLinks,
	} = useGoalAccountLinks(goalId);

	const [manageOpen, setManageOpen] = useState(false);
	const [view, setView] = useState<"timeline" | "contributions">(
		"contributions",
	);
	...
	const [selectedAllocation, setSelectedAllocation] =
		useState<GoalAllocation | null>(null);
	...
	const reloadDetails = async () => {
		await Promise.all([reloadAllocations(), reloadLinks(), reload()]);
	};

	const contributionData = useMemo(
		() => buildContributionData(allocations),
		[allocations],
	);
	...
	const timelineData = useMemo(() => {
		...
		const points: Array<{ month: string; savings: number; target: number }> =
			[];
	...
	const isLoadingAll = isLoading || allocationsLoading || linksLoading;

	if (isLoadingAll) {
		return <SavingsDetailsPageSkeleton />;
	}
		...
		return (
			<main className="min-h-screen bg-[#f7f6f4] p-6 dark:bg-[#171716] dark:text-white">
				...
				<p className="mt-3 text-gray-500">
					The Supabase goal ID in this URL does not exist or is not available to
					your account.
				</p>
				<Link
					href="/goals/savings"
	...
	const combinedError = error || allocationsError || linksError;
		...
		<main className="min-h-screen bg-[#f7f6f4] p-3 text-gray-950 sm:p-5 dark:bg-[#171716] dark:text-white">
			...
			<section className="mt-4 overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-white/5 dark:bg-[#232322]">
				<header className="flex flex-wrap items-center border-b border-gray-200 px-6 py-4 dark:border-white/10">
					<h2 className="text-xl font-bold">
						{view === "timeline" ? "Timeline" : "Contributions"}
					</h2>
			...
			<div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
					...
					<header className="flex items-center border-b border-gray-200 px-6 py-4 dark:border-white/10">
						...
						<div className="ml-auto flex items-center gap-3 text-sm">
							...
							<Toggle
								...
								onChange={setShowAdjustments}
								label="Show adjustments"
							/>
						</div>
					</header>
					{allocations
						.filter((a) => showAdjustments || a.kind !== "adjustment")
						.map((allocation) => {
							const account = savingsAccounts.find(
								(item) => item.id === allocation.accountId,
							);
							return (
								<button
									key={allocation.id}
								...
								>
									...
									<span className="ml-3 rounded-lg bg-gray-100 px-2 py-1 text-xs capitalize text-gray-500">
										{allocation.kind}
									</span>
									<span
				...
				<div className="space-y-4">
					<section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-white/5 dark:bg-[#232322]">
						<header className="border-b border-gray-200 px-5 py-4 text-xl font-bold dark:border-white/10">
							Allocation
						</header>
						{linkedAccounts.map((account) => (
							<div
								key={account.id}
								className="flex items-center border-b border-gray-200 px-5 py-4 last:border-b-0 dark:border-white/10"
							>
					...
					<section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-white/5 dark:bg-[#232322]">
						<header className="flex items-center border-b border-gray-200 px-5 py-4 dark:border-white/10">
							...
							<button
								...
								className="ml-auto font-semibold text-cyan-600"
							>
								Edit
							</button>
						</header>
						<div className="space-y-5 p-5">
							...
							<SummaryRow
								...
								value={`${formatCurrency(goal.monthlyContribution)}/mo.`}
							/>
						</div>
					</section>
				</div>
			...
			<AllocationDetailsSheet
				...
				account={
					savingsAccounts.find(
						(account) => account.id === selectedAllocation?.accountId,
					) ?? null
				}
			/>
			{confirmAction && (
...
function SavingsDetailsPageSkeleton() {
	return (
		<div className="min-h-screen bg-[#f7f6f4] p-3 sm:p-5 dark:bg-[#171716]">
			...
			{/* Hero image skeleton */}
			<Skeleton className="mt-4 h-[360px] w-full rounded-2xl sm:h-[410px]" />
