...
import { useUnifiedCategories } from "@/hooks/useUnifiedCategories"; // ✅ Added import
...
export function AddCategoryModal({
    ...
    const iconOptions = Object.keys(CATEGORY_HIERARCHY);
    const colorOptions = Object.keys(CATEGORY_COLORS);

    const [name, setName] = useState("");
    
    // Fallback to "Miscellaneous" or similar if options are empty to prevent crashes
    const [selectedIcon, setSelectedIcon] = useState(() => 
        parentCategory ? parentCategory : (iconOptions[0] || "Miscellaneous")
    );
    
    const [selectedColor, setSelectedColor] = useState(() => 
        parentCategory ? parentCategory : (colorOptions[0] || "Gray")
    );

    const addCustomCategory = useBudgetStore((state) => state.addCustomCategory);
    
    // ✅ Get master list for validation. We pass "Expense" and "All" 
    // just to get the full list of existing categories.
    const { allUnifiedCategories } = useUnifiedCategories("Expense", "All");

    // Dynamic Text Logic
    const isSubCategory = !!parentCategory;
    const titleText = isSubCategory ? `Add Sub-Category` : "Add Primary Category";
    const labelText = isSubCategory ? "Sub-Category Name" : "Primary Category Name";
    const placeholderText = isSubCategory ? "e.g. Sushi, Gas, Rent..." : "e.g. Lifestyle, Work, Home...";
    const buttonText = isSubCategory ? "Add Sub-Category" : "Add Primary Category";
    ...
    const handleAdd = async () => {
        ...
        if (isDuplicate) {
            return alert(`The category "${trimmedName}" already exists!`);
        }
            ...
            <div className="bg-[#0d0d0d] border border-white/10 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
                                ...
                                {iconOptions.map((icon) => (
                                    ...
                                    >
                                        <CategoryIcon 
                                            name={icon} 
                                            size={18} 
                                            colorClass={selectedIcon === icon ? "text-orange-500" : "text-gray-400"} 
                                        />
                                    </button>
                                ))}
                            </div>
                ...
                <div className="p-6 bg-white/2 border-t border-white/5 flex justify-end gap-3">
                    ...
                    <button
                        ...
                        className="px-8 py-2 bg-orange-600 hover:bg-orange-500 disabled:opacity-30 disabled:cursor-not-allowed text-white text-sm font-black rounded-xl transition-all shadow-lg shadow-orange-600/20 active:scale-95"
                    >
                        {buttonText}
                    </button>
                </div>
