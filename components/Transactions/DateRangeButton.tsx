...
function buildCalendarDays(month: Date): CalendarDay[] {
	...
	while (cells.length % 7 !== 0)
		cells.push({ key: `end-${cells.length}`, date: null });
	return cells;
}
		...
		<div className="min-w-0">
			...
			<div className="grid grid-cols-7">
				{buildCalendarDays(month).map((cell) => {
					...
					const selected =
						sameDate(cell.date, start) || sameDate(cell.date, end);
					...
					return (
						<button
							...
							className={`aspect-square border border-gray-200 text-base transition-colors dark:border-white/10 ${selected ? "bg-[#00A8D2] font-bold text-white" : inside ? "bg-cyan-50 text-cyan-900 dark:bg-cyan-500/15 dark:text-cyan-100" : "hover:bg-gray-100 dark:hover:bg-white/5"}`}
						>
							{cell.date.getDate()}
						</button>
					);
...
export function DateRangeButton({
	value,
	onChange,
	active = Boolean(value.startDate || value.endDate),
	variant = "toolbar",
	onBeforeOpen,
	className = "",
	...
	const selectDate = (date: Date): void => {
		...
		setDraft((current) => {
			...
			if (iso < current.startDate)
				return { startDate: iso, endDate: current.startDate };
		...
		>
			<Popover.Trigger asChild>
				{variant === "toolbar" ? (
					...
					<button
						type="button"
						className={`grid h-13 w-full grid-cols-[1fr_auto_1fr] items-center rounded-xl border bg-white text-left text-base outline-none dark:bg-[#222] ${open ? "border-[#00A8D2] ring-2 ring-[#00A8D2]/15" : "border-gray-300 dark:border-white/15"}`}
					>
						<span className="truncate px-4 text-gray-500 dark:text-gray-400">
							{formatLabel(value.startDate, "Start date")}
						</span>
						<span className="text-xl text-gray-500">→</span>
						<span className="truncate px-4 text-gray-500 dark:text-gray-400">
							{formatLabel(value.endDate, "End date")}
						</span>
					</button>
				)}
			</Popover.Trigger>
			<Popover.Portal>
				<Popover.Content
					...
					sideOffset={10}
					collisionPadding={16}
					className="z-[9999] w-[min(930px,calc(100vw-24px))] overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl outline-none dark:border-white/10 dark:bg-[#1B1B1B]"
				>
							...
							{DATE_PRESETS.map((preset) => {
								...
								return (
									<button
										...
										className={`block w-full rounded-lg px-3 py-2.5 text-left text-base font-medium ${dateRangesEqual(range, draft) ? "bg-[#FF5A35]/10 text-[#FF5A35]" : "hover:bg-gray-100 dark:hover:bg-white/5"}`}
									>
										{preset.label}
									</button>
								);
						...
						<div className="min-w-0 p-6">
							<div className="mb-4 flex justify-between">
								<button
									...
									onClick={() =>
										setLeftMonth((current) => addMonths(current, -1))
									}
							...
							<div className="grid gap-8 lg:grid-cols-2">
								...
								<CalendarMonth
									...
									draft={draft}
									onMonthChange={(right) => setLeftMonth(addMonths(right, -1))}
									onSelectDate={selectDate}
								/>
							</div>
							{invalid && (
					...
					<div className="flex items-center justify-between border-t border-gray-200 px-6 py-4 dark:border-white/10">
						<button
							...
							className="rounded-xl border border-gray-200 px-5 py-3 font-semibold dark:border-white/10"
						>
							Clear
						</button>
						<div className="flex gap-3">
							...
							<button
								type="button"
								disabled={invalid || dateRangesEqual(draft, value)}
								onClick={() => {
									if (!invalid) {
										onChange(draft);
										setOpen(false);
								...
								className="rounded-xl bg-[#FF5A35] px-5 py-3 font-bold text-white disabled:opacity-45"
							>
								Apply
							</button>
						</div>
